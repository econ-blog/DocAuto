...
5. 사용자가 채팅으로 정답을 준다 — 텔레그램 인박스 형식 그대로의 `제품명 시퀀스`(예: `징코샷 O24`)든, `제품명 정답` 형태의 직접 지시(예: `시너지아정 시트룰린`)든 — 절차는 [MEMORY.md](MEMORY.md) "채팅으로 정답 받기" 참고.

**정답/문제은행 반영은 확인 없이 바로 main에 merge·push한다.** 위 1·2·5 어느 경로로 답을 채우든(no_answer 보충, incomplete_bank 족보 채움, 채팅으로 직접 받은 정답) 지정 브랜치에 커밋한 뒤 **별도 확인 없이 바로** `main`에 merge하고 push하는 것이 기본값이다(2026-08-24 사용자 지시로 도입 — 최초엔 채팅 시퀀스 입력에 한정됐으나, 2026-08-27 사용자 지시로 정답 수정 작업 전반으로 확장). 예전엔 명시적 요청 시에만 반영했다.
