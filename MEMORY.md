...
### ① 세미나 표의 계정 분리 — **결정 필요, 미착수**

사용자 질문(2026-08-28): "세미나 표도 승진버전과 원주버전이 있어야하지 않아?"

현재는 두 계정을 한 칸에 합치고 **나쁜 쪽이 이긴다**(`runlog.merge_status`). 그래서
승진만 실패하고 원주는 성공한 경우 ❌ 하나만 보이고 어느 계정인지 알 수 없다.
daily 표는 이미 계정별 컬럼(`출석 승진` / `출석 원주`)으로 나뉘어 있어 **세미나 표만 예외**다.

| 안 | 형태 | 장단 |
|---|---|---|
| **A (권장)** | 컬럼을 6개로: `신청 승진` `신청 원주` `입장 승진` `입장 원주` `설문 승진` `설문 원주` | daily 표와 일관됨. 표 1장 유지. 총 9컬럼이라 폭은 감당 가능 |
| B | 계정마다 PNG 1장씩 총 2장 | 각 표는 좁아지지만 알림이 2배 |
| C | 한 칸에 `✅/❌`처럼 두 이모지 | 컬럼 수 그대로지만 어느 쪽이 어느 계정인지 규칙을 외워야 함 |

**착수 지점:** `runlog.SEMINAR_PHASES`와 `runlog.seminar_table()`. 로그 스키마는 이미
계정별로 쌓고 있다(`{"apply": {"bjh7790": "success", "wonju": "failed"}}`) — **데이터는
그대로 두고 렌더링만 바꾸면 된다.** `merge_status`는 daily 표에서 계속 쓰이므로 남길 것.

### ② 목록 제목 추출 — 미검증

2026-08-28 수정으로 제목을 목록(`SEMINAR_LIST_JS`)에서 가져오지만 **실제 런에서 아직
확인되지 않았다**(같은 날 SyntaxError로 신청 스텝이 죽어서 한 번도 못 돌았다).
다음 세미나 블록 런의 표에서 실제 세미나 이름이 나오는지 볼 것. `세미나 5583`처럼
번호로 나오면 목록 셀렉터(`.tit, dt, .title, strong`)도 이 사이트엔 안 맞는 것이고,
그때는 `recon`으로 목록 DOM을 떠야 한다.
