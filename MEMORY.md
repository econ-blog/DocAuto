...
| 확인 대상 | 어디서 | 기대값 | 아니면 |
|---|---|---|---|
| 목록 일시 파싱이 되나 | `seminar` 노드 `listed_today` | 그날 방송 세미나 수(2026-09-07이면 7) | 0이면 정규식이 목록 표기와 안 맞는 것. `raw` 원문을 확인해야 한다 — 목록 항목 innerText는 결과 JSON에 안 남으므로 `RECON=1` 또는 `--headed`로 한 항목만 떠 본다 |
...
지금은 남기지 않는다(개인정보 우려는 없지만 로그 크기·커밋 소음). 한 번이라도
`listed_today`가 0으로 나오면 그때 샘플 1건 로깅을 넣는다.
