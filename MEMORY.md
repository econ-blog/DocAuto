...
### legacy 파일 잔여 현황 (2026-09-02 실측)

`quiz_answers_legacy.json` — 6키. 앞선 기록(`리토바`·`대웅징코샷`)은 이미 빠졌다.

| 키 | 값 | `quiz_answers.json` 대응 | 판단 |
|---|---|---|---|
| `렉사프로` | `ooo` | 없음 | 답이 legacy에만 있다. 유지 |
| `루피어데포` | `314` | 없음 | 〃 |
| `뮤코트라` | `232` | 없음 | 〃 |
| ~~`아림시스`~~ | `112` | `아림시스주`(3문항) | **2026-09-02 삭제**(사용자 지시) |
| ~~`시너지아`~~ | `144` | `시너지아정`(3문항) | 〃 |
| ~~`엔블로`~~ | `113` | `엔블로`(4문항) | 〃 |

삭제 후 남은 키는 `렉사프로`·`루피어데포`·`뮤코트라` 3개다. 셋 다 `quiz_answers.json`에
대응 항목이 없어 답이 legacy에만 있다.

삭제한 셋의 리스크: 닥터빌이 접미사 없는 표기(`아림시스`/`시너지아`)로 렌더하면
조회가 빗나가 `no_answer`가 뜬다. 오답을 내는 게 아니라 다시 물어보는 것이라
감수 가능하다고 판단했다. 그때는 신형식 키에 문항을 추가한다.

`survey_answers_legacy.json` — 72키(도입 시 130키에서 감소, 즉 `apply_promotions` 승격은
실제로 돌고 있다). 이 중 59키가 general 문항의 `"2"`라 앞으로 조회되지 않는다.
남은 13키만이 자연 배수 대상이다(선택형 숫자 10 + 척도 1 + 주관식 2).
퀴즈·주관식이 다 빠지면 남은 `"2"` 뭉치를 한 번에 버리고 파일을 삭제한다.
...
- **복합 문항(보기 + 입력란, `kind="mixed"`, 2026-09-09):** DOM 판독기가
  `options.length ? 'choice' : …`로 접는 바람에 보기가 하나라도 있으면 같은 `li`의
  `textarea`를 통째로 버렸다. 그 문항은 `general`로 분류돼 2번 보기만 찍고 주관식
  칸은 빈 채로 제출됐다 — 족보에 오르지도 `incomplete_bank`로 막히지도 않아
  드러나지 않는 사고였다. 지금은 `mixed`로 읽어 선택 파트(퀴즈/일반 규칙)와 입력
  파트(text 족보)를 따로 풀고, 한쪽이라도 미등록이면 페이지를 막는다.
  입력란 name은 `free_name`으로 따로 싣는다.
  - 제출 직전 "빈 입력란이 남았으면 중단"이라는 범용 안전망은 **넣지 않았다.**
    `(빈칸)`(`BLANK_ANSWER_MARKER`)으로 일부러 비워 제출하는 주관식이 실제로 있다
    (방문 요청 병원 정보 등). 안전망을 두면 그것들이 전부 막힌다.
- **`not_ready`의 이중 의미 (2026-09-09 수정):** 창 전과 "창은 열렸는데 못 열었다"를
  한 상태에 담고 있었다. 후자는 `quiet`이라 알림이 안 갔다 — 세미나 5627에서 같은
  런의 bjh7790은 success, wonju만 `not_ready`로 조용히 빠졌다. 손으로 재시도해
  붙였을 뿐, 안 봤으면 21:00 마감까지 한 계정 누락으로 끝났다. 이제
  `unopened_status`가 창이 열려 있는 동안의 실패를 `unverified`(alert)로 올린다.
  창 전은 그대로 `not_ready`, 마감 후는 `closed`.
- 미기입 항목 조회는 `scripts/bank_pending.py`. 판정 규칙을 재구현하지 않고
  `doctorville.coerce_bank_answer` / `seminar_survey._coerce_answer`를 그대로 부른다
  (닥터빌은 보기 한 줄만 정답, 설문은 복수 선택이라 여러 줄도 정답 — 규칙이 다르다).
...
- **www 상세는 사실상 언제나 '진행 중'으로 읽혔다 (2026-09-17 세미나 5666, run 35210903336, 수정함):** 19:37 런에서 wonju는 설문 창을 열었는데 bjh7790만 팝업 2회 시도 후 실패했고, 그 판정이 quiet `not_ready`로 떨어졌다(엇갈림 승격이 뒤늦게 `unverified`로 올렸다). 같은 시각 실측 버튼 —
  - m `/cme/vod/5666`: `뒤로 가기 / 관심 추가 / 공유 / 설문참여 / 세미나 종료` (**`설문참여` 버튼은 2026-08-31 실측에 없던 것이다.** 설문이 열려 있는데 못 열었다는 증거이기도 하다. `SURVEY_PENDING_MARKERS`에는 넣지 않았다 — `설문 참여 완료`의 부분 문자열이라 본문 대조에서 완료를 미참여로 뒤집는다.)
  - www `?seminarId=5666`: `전체 메뉴 열기 / 관심 / 재입장하기 / 목록 / 닥터빌 라이브세미나를 통해 선생님의 노하우를 공유해보세요! / 커뮤니티 / …`
  - **결함 ①: `seminar_running`이 홍보 문구의 '라이브'를 물었다.** www는 홍보 문장까지 `[class*=btn]`로 실어 오는데 거기에 `라이브`가 들어 있다. 문구가 늘 있으니 **www 조회는 세미나가 끝났든 말든 항상 `running=True`**였다. → 진행 중 표식은 **버튼 라벨에서만** 읽는다(공백 제거 후 `RUNNING_LABEL_MAX_LEN`=12자 이하인 텍스트).
  - **결함 ②: '재입장하기'에 '입장하기'가 들어 있다.** 다시보기 입장 버튼이라 끝난 세미나에도 남는다. → `SEMINAR_NOT_RUNNING_MARKERS`로 대조 전에 지운다.
  - **결함 ③: m의 '세미나 종료' 관측까지 같이 버려졌다.** m VOD에는 `로그아웃`·`마이페이지`가 없어 미참여 판정이 보류되는데(위 "m VOD 상세의 버튼 구성"), 그 보류가 `usable=False`로 이어져 **종료 관측도 사라졌다.** 페이지 신원(URL)은 이미 확인한 뒤이고 `세미나 종료`는 로그인과 무관한 사이트 상태다. → 보류하는 것은 설문 참여 판정뿐이고, `ended`/`running` 기록은 살린다(`rec["usable"]=True` 후 판정만 `unknown`).
  - 셋이 겹쳐 "m은 종료를 보여 주는데 www만 진행 중"이 되고, `unopened_status(running=True)`가 alert를 quiet으로 덮었다. 고친 뒤에는 엇갈림 승격에 기대지 않고 그 자리에서 `unverified`가 난다.
  - **경위(사용자 지적, 2026-09-17):** 5666 공지는 **18:30~20:00**인데 wonju가 19:37에 설문 창을 열었다 — 실제로는 19:36쯤 끝난 것이다. 계정은 bjh7790 → wonju 순서로 도니 **bjh7790은 종료 1~2분 전에 팝업을 시도**했고(그래서 못 열렸다), 그 뒤 상세를 조회할 때는 이미 끝나 있었다(`세미나 종료`+`설문참여`). 팝업 실패 자체는 결함이 아니다. 결함은 **끝난 것을 보고도 quiet으로 덮은 것**이고, 창은 실제 종료 +1시간이라 다음 블록(20:00)이 다시 집으면 된다.
  - **위 오독을 걷어내면 방송 중 정상 실패가 alert로 샌다.** 홍보 문구의 '라이브'가 www 조회를 항상 `running=True`로 만들어 온 덕에 방송 중 팝업 실패가 조용했던 것이다(이미 입장한 계정은 방송 중에도 `재입장하기`로 보일 수 있다). → `unopened_status`에 그물을 하나 더 뒀다: **종료를 한 번도 못 봤고(`observed_end` 없음) 공지된 종료를 알고 있으며 그 종료 + `SURVEY_RUNNING_GRACE`가 안 지났으면 quiet `not_ready`.** 침묵의 근거를 "진행 중 표식을 찾았다"가 아니라 **"끝난 것을 본 적이 없다"**에 둔다. 일정을 아예 모르면 이 그물은 쓰지 않는다 — "모른다"는 "방송 중"이 아니다(2026-09-09 5627).
...
## 설문 1페이지 진행 막힘 (2026-09-07 세미나 5616) — **원인 미확정**

세미나 5616(전공의를 위한 호흡기내과 증례강의) 설문이 bjh7790에서
`1페이지 진행 후에도 같은 문항이 다시 표시됨 — 중단.`으로 죽었다.
5587(응급실 증례강의)과 같은 시리즈·같은 템플릿이다.

### 남아 있는 관측 (run 34085217034)

결과 JSON에 있던 것은 `static_items: 10`, `pages: 1` 두 개뿐이었다.
스크린샷 `survey_5616_stuck_*.png`는 artifact에 있었으나 **에이전트 환경의 egress
정책이 blob 스토리지를 막아 받지 못했다.** DOM 덤프는 빈 문항 분기에서만 뜨는데
5616은 텍스트가 읽혔으므로 애초에 남지 않았다. 즉 **원인을 좁힐 증거가 없었다.**

사용자 육안 확인(설문 실물):

| 페이지 | 내용 |
|---|---|
| 1 | 1번 = 선택형(2번), 2~10번 = **활성화되어 있지 않음**, 11번 = 선택형(2번) |
| 2 | 객관식 여러 건 |
| 3 | 주관식 1건 (펙수프라잔 — `survey_text_answers.json`에 등록함) |

즉 페이지 1에서 답해야 하는 것은 1·11번뿐이고 스크립트도 그 둘을 채웠다.
그런데도 진행이 안 됐다. 미응답 문항이 원인이라는 설명은 사용자 관측과 맞지 않는다.

### 이번에 바꾼 것

| 변경 | 위치 | 근거 |
|---|---|---|
| 고정 5초 대기 → 페이지가 바뀔 때까지 폴링 | `wait_for_page_change` | 렌더가 5초보다 늦으면 같은 페이지를 읽고 막힘으로 오판했다. 대부분은 1초 안에 바뀌므로 평균 대기는 오히려 줄어든다 |
| 안 넘어가면 답 재적용 후 1회 재클릭 | `run_survey` (`ADVANCE_ATTEMPTS`) | 페이지가 그대로 = 제출이 안 된 것이므로 중복 제출이 아니다 |
| 닫은 알림 문구를 결과에 보존 | `result["alerts"]` | 막힘의 이유가 적혀 있는데 `dismiss_alerts` 반환값이 버려지고 있었다 |
...
### ② 마감 세미나가 표에 뜨는지 — **2026-09-07 확정·수정됨**

가설 후자가 맞았다. 세미나 5657(New WAVE Webinar, 09-07 13:00~14:00)이 **정원 마감**이라
목록 항목에 `span.ico_apply`가 붙지 않았고, `applicable=False`로 `seminar_ids`에서 빠져
상세를 열지 않았다. 상세를 열지 않으니 2026-08-28에 넣은 `closed` 경로도 타지 못했다 —
행이 아예 없어 표에서 사라졌고, 사용자가 눈으로 보고서야 발견했다.

**수정(2026-09-07):** 표 행의 출처를 신청 이력에서 **목록**으로 넓혔다.
`SEMINAR_LIST_JS`가 항목 innerText를 `raw`로 함께 반환하고,
`parse_list_datetime` → `list_rows_for_today`가 **오늘 방송분 전부**를 신청 여부와 무관하게
행으로 만든다(페이지 로드 0회). 원칙 세 가지:

- **파싱은 fail-closed.** 목록 일시 표기가 상세 `dd.date`와 같다는 보장이 없어 원문을
  방어적으로 판다. 날짜·시각을 못 찾으면 행을 만들지 않고 `list_unparsed`로 세기만 한다.
  실패했는데 행을 만들면 수 주치 세미나가 오늘 표에 쏟아진다.
- **연도 없는 표기(`09.07`)는 오늘에서 가장 가까운 연도로 추정한다.** 12월/1월 경계 방지.
- **`seminar_applied.json`은 목록 값으로 덮어쓰지 않는다.** 그 값은 `seminar_live`의 입장
  윈도우 계산에 쓰이고, 목록 표기는 상세 `dd.date`보다 신뢰도가 낮다. 목록 시각은 표
  로그에만 반영한다 — 이력의 시각이 신청 시점에 고착되는 문제(5613: 이력 12:00, 실제
  12:30)도 이 경로로 표에서만 교정된다.
- **마감 세미나에 `closed`를 박아 재시도를 끊지는 않는다.** 정원 마감은 취소로 되돌아오는
  상태라 신청 재시도는 유지한다. 상태 없는 행은 표에서 ❔ — "자동화가 신청하지 못함"의 신호.

### ②-old 당시 기록 (2026-08-28)
...
## 거짓 `already_done` — 남의 페이지를 읽고 있었다 (2026-09-11 확인·수정)

**증상 2건.**
- 세미나 5639(리브레, 18:30~20:00 공지): 제출한 적 없는데 bjh7790이 `already_done`.
  같은 런에서 wonju는 공지 기준 마감(21:00)에 걸려 `closed`.
- 세미나 5696(카보메틱스, 19:00~22:10): **끝나기 전인 21:05**에 wonju가 `already_done`.
  같은 런의 bjh7790은 `unverified`. 5694(09-10 20:00~22:00)도 21:35에 같은 모양.

**진단.** 같은 세미나·같은 시각인데 계정마다 판정이 갈렸다 = 세미나 상세가 아니라
**계정마다 내용이 다른 페이지**를 읽었다는 뜻이다. m 상세는 `/cme/vod/{id}`인데
아직 VOD가 없는 회차는 목록으로 떨어지고, 그 목록에 들어 있던 **다른 세미나의
'응답완료'**가 `matched_done_marker`의 부분 문자열 검색에 걸렸다. `is_mobile_session`은
호스트와 안내 문구만 봐서 목록 페이지를 걸러내지 못했다. 게다가 버튼 판정이
`unknown`이면 `detect_survey_marker([body])`로 **본문 전체**를 다시 봤는데, 이건
`read_detail_buttons`가 "보이는 버튼만 본다"로 막아 둔 오탐 경로를 그대로 되살린다.

**수정 3겹** (`seminar_survey.py`):
1. `detail_url_matches` — 최종 URL에 그 세미나 id가 없으면 판정을 통째로 버린다.
   id 없는 주소(`/cme/vod`, `/cme/vodList`)는 목록이다.
2. `detect_survey_marker(..., allow_done=False)` — 본문 전체로는 `done`을 만들지
   않는다. 거짓 `done`은 상태에 굳어 재시도조차 안 되고, 거짓 `not_done`은 다음 런이
   회복한다. 비용이 비대칭이므로 판정도 비대칭이어야 한다.
3. `seminar_has_ended` — 아직 안 끝난 세미나의 `done`은 폐기하고 `unverified`로 둔다.

## 설문 창 = **실제 종료 ~ 실제 종료 + 1시간** (2026-09-11 사용자 지시)

공지된 일정은 **양쪽으로 다 틀린다.**
- 5639(리브레): 18:30~20:00 공지인데 21:00 넘어서도 방송 중. 공지 기준 마감(21:00)에
  걸려 wonju가 `closed`로 조용히 떨어졌다.
- 5694(Global Journal Club): 20:00~22:00 공지인데 **21:05에 이미 설문이 열려 있었다.**

그래서 창의 기준점은 공지가 아니라 관측된 실제 종료(`survey_meta[sid].ended_at`)다.

| 관측 | 판정 |
|---|---|
| `ended_at` 있음 | `now < ended_at` → `not_ready` / `ended_at+1h` 경과 → `closed` / 그 사이 → `ready` |
| `ended_at` 없음 | 공지 시작+30분 전 → `not_ready`(요청 절약 게이트) / 이후 → `ready` / 공지 시작+12h 경과 → `closed`(stale 상한) |

**`ended_at`은 두 경로로 기록되고, 가장 이른 관측만 남긴다**(나중 관측으로 덮으면 창이
통째로 밀린다):
1. 설문 창이 실제로 열렸다 — 설문은 세미나가 끝나야 열리므로 그 자체가 종료의 증거다.
2. 상세에 '세미나 종료'가 떴다(`probe_saw_ended_seminar`).

공지된 **종료** 시각은 어느 판정에도 쓰지 않는다. 공지 **시작**은 사전 게이트에만 쓴다 —
종료를 확인하려면 어차피 한 번은 열어 봐야 하고, 시작 30분 전에는 열어 볼 이유가 없다.
요청량은 종전과 같다.

부수 효과: 상세가 "아직 안 끝났다"고 말하는데 창이 안 열리면 `unverified`(alert)가 아니라
`not_ready`(quiet)다. 설문은 원래 그때 안 열린다 — 실패가 아니다.

### `done` 재검증은 넣지 않는다 (2026-09-11 사용자 판단)

닥터빌 요청이 늘면 `net::ERR_CONNECTION_CLOSED`가 늘어난다(errors 로그에 이력 있음).
게다가 state 캐시 키가 `seminar-state-${KST_DATE}-`라 **날짜가 바뀌면 상태가 비므로**,
잘못 굳은 `done`은 자정에 자연 소멸한다. 주기적 재검증의 값어치가 없다.
...
## 런 검증 체크리스트 (2026-09-02 CLAUDE.md에서 이관)

CLAUDE.md는 규칙만 유지한다. 날짜별 "다음 런에서 확인할 것"·인계·미결 항목은 아래에 원문 그대로 보존한다.
...
알려진 한계 (사용자 확인함, 2026-08-12): 마감·정원초과 세미나는 신청 이력에 기록되지 않아 매 런 상세를 다시 연다. 그런 세미나가 많지 않다고 판단해 그대로 둔다. **의도한 낭비다** — 정원 마감은 취소로 풀릴 수 있어 재시도를 끊지 않는다(2026-09-07 재확인). 표 누락 문제는 목록 기반 행으로 별도 해결했다(인계 ② 참고).
...
| 세미나 행 구성 | 세미나 표 | 그날 예정 세미나가 전부 행으로 (신청 못 한 마감분은 상태 없는 ❔ 행) | 비면 목록 일시 파싱 실패 — 결과 JSON의 `listed_today`·`list_unparsed`를 본다. `list_unparsed`가 크면 목록 표기가 정규식과 안 맞는 것 |
| 세미나 제목 | 세미나 표 | 실제 세미나 이름 (번호 아님) | 입장·설문한 세미나는 08-28에 검증됨. **신청만 한 세미나가 번호로 나오면** 신청 스텝이 목록에서 못 본 것 — MEMORY.md 인계 ①② 참고 |
| 세미나 표 계정 컬럼 | 세미나 표 | `신청 승진` `신청 원주` … 9컬럼, 계정별로 다른 이모지가 보임 | 컬럼명이 계정키(`bjh7790`)로 나오면 credentials 라벨 미적용, `신청` 한 칸이면 계정 기록이 안 쌓인 것 |
...
## 다음 런에서 확인할 것 (2026-08-28 설문 판정·표 유지 변경분)

**실제 런에서 아직 검증되지 않았다.** 다음 `seminar_block` 결과에서 아래를 본다.

| 확인 대상 | 어디서 | 기대값 | 아니면 |
|---|---|---|---|
| 설문 성공의 증거 | 설문 결과 JSON | `verified_by: "detail_button: 설문 참여 완료"` | `unverified`가 나오면 **결과 JSON의 `detail_buttons`를 본다** — www(데스크톱) 상세의 실제 문구가 거기 다 실려 온다. 사용자 실측 화면은 m(모바일)이라 문구가 다를 수 있다 |
| 창이 닫혀도 성공 판정 | 설문 결과 JSON | 예전엔 무조건 `unverified`였던 케이스가 `success`로 | 그대로 `unverified`면 상세 문구부터 확인 |
| 이미 참여 판별 | 설문 결과 JSON | 설문 창이 안 열리는데 상세엔 완료 표시 → `already_done` | 계속 `not_ready`만 나오면 상세 판정이 안 걸린 것 |
| 세미나 표의 ✅ 유지 | 세미나 표 | 2번째 런부터도 앞 런의 ✅가 ☑️로 안 바뀜 | ☑️로 되돌아가면 `NON_OVERWRITING_STATUSES` 미적용 |

---

## 다음 런에서 확인할 것 (2026-08-31 설문 완료 판정 모바일 우선)
...
## 다음 런에서 확인할 것 (2026-09-07 목록 기반 행 도입분)

**실제 런에서 검증되지 않았다.** 목록 DOM의 실제 일시 표기를 눈으로 본 적이 없고,
`parse_list_datetime`의 정규식은 그 표기를 추측해 짠 것이다. 다음 `daily` 런의
`seminar` 노드에서 아래를 본다.

| 확인 대상 | 어디서 | 기대값 | 아니면 |
|---|---|---|---|
| 목록 일시 파싱이 되나 | `seminar` 노드 `listed_today` | 그날 방송 세미나 수(2026-09-07이면 7) | 0이면 `list_raw_samples`(실패 원문 3건)를 본다. 항목 전부 실패면 `list_blind: true`로 `unverified` 알림이 온다 |
| 파싱 실패 규모 | `seminar` 노드 `list_unparsed` | 작을수록 좋음 | 크면 목록에 일시가 없는 항목(배너·기획전)이 섞였거나 정규식이 절반만 맞는 것 |
| 마감 세미나가 표에 뜨나 | 세미나 표 | 신청 못 한 세미나가 상태 없는 ❔ 행으로 존재 | 안 뜨면 그 세미나가 목록에 아예 없다는 뜻 — 그때는 목록 페이지네이션·지연 렌더를 의심한다 |
| 시각 고착 교정 | 세미나 표의 시각 | 목록 값과 일치(이력 값이 낡았어도) | 낡은 값이 보이면 `list_starts` 우선 경로가 안 탄 것 |

**`listed_today`와 `list_unparsed`는 서로를 검증하지 못한다.** `list_unparsed`가 커도
오늘 방송분만 정상 파싱되면 `listed_today`는 멀쩡해 보인다 — "목록에 일시 없는 항목이
섞였다"와 "파서가 절반 실패했다"를 구분하려면 실패한 `raw` 샘플이 필요하다.
**2026-09-15에 `listed_today` 0이 확인되어 샘플 로깅을 넣었다** — `list_raw_samples`
(최대 3건, 각 120자). 아래 절 참조.

---

## 목록 스캔이 통째로 실패한다 (2026-09-15 확정)

### 관측

`펙수클루 Triple Symposium`(5683)이 17:05 세미나 블록 표에 없다가 17:36 표에 나타났다.
추적 결과 신청 기록이 **한 번도** 없었다.

| 런 | `list_items` | `listed_today` | `list_unparsed` | 신청 대상 | 신청 |
|---|---|---|---|---|---|
| 09-10 00:19 | — | — | — | 있음 | 11건 |
| 09-13 00:15 | 64 | 0 | 64 | 0 | 0 |
| 09-14 22:38 | — | — | — | 있음 | 4건 |
| 09-15 00:15 | 63 | 0 | 63 | 0 | 0 |

`a.list_detail`은 63건 잡히는데 `span.ico_apply` 0건 + 일시 텍스트 0건. 두 신호가
**동시에** 없다 = 항목 껍데기만 읽었다는 뜻이다. 같은 페이지를 읽는
`seminar_live.get_live_seminar_info`는 1500ms를 기다리고 정상 동작했다(`ico_enter`로
입장 대상을 계속 찾아냈다). `task_seminar`는 1000ms 고정 sleep이었다 — SPA 렌더 레이스.

**실패가 조용했던 것이 더 큰 문제다.** 신청 대상 0건 → `no_target`(quiet) → 텔레그램
없음. 며칠째 신청 0건인 것을 아무도 몰랐다.

### 기록 없이 입장된 세미나 9건

전체 git 히스토리의 세미나 로그를 훑어, `apply` 칸이 한 번도 없었던 세미나:
5653 · 5657 · 5664 · 5673 · 5681 · 5683 · 5690 · 5694 · 5696.
5657(정원 마감)을 뺀 8건은 **입장까지 성공**했고, 9건 모두 `seminar_applied.json`에
없다. 하루 한 건꼴의 상시 누락이다.

신청을 **시도**했다면 성공이든 `unverified`든 `closed`든 `_log_seminar`가 `apply` 칸을
남긴다. 그게 없다 = 상세를 연 적조차 없다. 즉 `seminar_applied.json` 쓰기 경로의
문제가 아니라 목록 스캔이 그 세미나를 못 본 것이다.

**자가 치유가 구조적으로 불가능하다.** 기록이 한 번 빠지면 → 이미 신청된 세미나라
`ico_apply`가 안 붙고 → `seminar_ids`에서 빠지고 → 상세를 안 열어 `신청취소` 버튼으로
뒤늦게 기록할 기회가 없다. 구멍이 영구화된다.

### 아직 못 가린 것

사용자 확인: **신청 없이는 입장 버튼이 안 뜬다.** 그러면 위 8건은 어딘가에서 신청된
것이다. 남은 가설 둘:

1. 스캔이 멀쩡했던 런에서 신청됐는데 그 런의 기록이 유실 — 샘플로 본 실패 런
   (09-05·09-06·09-07·09-09·09-12)은 커밋 스텝이 모두 성공했다. 전수는 아니다.
2. **사이트의 승계 신청.** 5690은 `[재]`(재방송), 5683은 `Triple Symposium`(펙수클루
   묶음)이고 이력에는 신경과·소화기내과·심장내과 펙수클루가 각각 있다. 원 세미나
   신청이 재방송·묶음에 승계되면 클릭도 기록도 없이 입장만 열린다 — 관측과 일치한다.

가르는 법: 5683 상세의 버튼 문구를 본다. 기록이 없는데 `신청취소`면 2번 확정.

### 수정 (2026-09-15)

- `wait_for_seminar_list`: 고정 sleep 대신 항목 innerText에 시각 표기(`\d{1,2}:\d{2}`)가
  나타날 때까지 `wait_for_function`(8초). 실패해도 예외를 던지지 않고 1500ms 폴백 후
  스캔한다 — 진짜로 항목이 없는 날과 레이스를 대기만으로는 못 가르기 때문이다.
- `list_blind` 판정: 목록 항목이 있고, **전부** 일시 파싱에 실패했고, 신청 가능 배지도
  0건이면 `unverified`(alert). `no_target`(quiet)으로 새지 않는다. 항목 0건이거나 일시가
  읽히면 종전처럼 `no_target`이다(마감·이미 신청한 날의 정상 상태).
- `list_raw_samples`: 파싱 실패 항목 원문 최대 3건(각 120자)을 결과 JSON에 남긴다.
  정규식이 왜 안 맞는지 사후에 보려면 이것뿐이다 — 지금까지는 매번 `--headed`로
  다시 떠야 했다.
- 미해결: 입장은 했는데 이력에 없는 세미나를 되메우는 화해(reconcile) 경로. 위
  가설 2가 맞으면 승계 신청분은 영영 이력에 안 들어오므로, 표 행 출처를 이력이 아니라
  입장 이력까지 포함하도록 넓히는 쪽이 맞다.

---

## 다음 세션 인계 (2026-08-28)

이어서 할 일은 전부 [MEMORY.md](MEMORY.md) **"다음 세션에서 이어서 할 일"**에 있다. 요약:
...
원래부터 계정별이라 렌더링만 바꿨다. 설계·폴백 규칙은 [MEMORY.md](MEMORY.md)
"세미나 표의 계정 분리". 08-28 실제 로그로 재렌더까지는 확인했고 **실제 런은 미검증** —
아래 표에서 확인할 것.

**2026-08-28 사고:** 목록 제목 추출 JS를 일반 문자열에 넣어 `\n`이 줄바꿈이 되는 바람에
두 계정 세미나 신청이 전부 `SyntaxError`로 죽었다. `SEMINAR_LIST_JS`를 r-문자열로 바꿔
수정했고 `tests/test_embedded_js.py`가 재발을 막는다. **브라우저에 넘기는 JS는 r-문자열로 쓸 것.**
...
### ~~② 정찰 R1 미수집 — 설문 증거 판정이 아직 휴리스틱~~ — 2026-08-28 무의미해짐

R1은 **설문 완료 화면 문구**를 모으기 위한 것이었다. 그런데 완료 판정 자체가 완료 화면에서 **세미나 상세 재접속**으로 옮겨갔다(사용자 지시). 상세에 `설문 참여 완료` 버튼이 있으면 성공, `세미나 종료`만 있으면 미참여다 — 문구를 모을 필요가 없다. 상세는 [MEMORY.md](MEMORY.md) "설문 완료 표식".

R1 덤프 코드(`RECON=1`)는 그대로 두었으나 판정 근거는 아니다. 산출물은 `scripts/logs/recon_*`에 남고 artifact로만 올라간다 — **커밋 금지**(설문 페이지에 이름·소속 포함).

| ID | 대상 | 상태 |
|---|---|---|
| ~~R1~~ | ~~설문 완료 화면 문구~~ | **2026-08-28 불필요** — 상세 페이지 버튼으로 판정 |
