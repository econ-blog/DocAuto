...
## 설문 마감 = 공지 종료가 아니라 **실제 종료 관측 + 1시간** (2026-09-11 사용자 지시)

공지된 종료 시각을 넘겨 진행되는 세미나가 흔하다(5639는 20:00 공지인데 21:00 넘어서도
방송 중이었다). 공지만 보고 끊으면 아직 열려 있는 설문을 조용히 버린다.

- 상세를 읽었는데 '세미나 종료'가 **없으면** 그 시각을 `survey_meta[sid].running_at`에
  적는다(`probe_saw_running_seminar` → `mark_survey_running`).
- 마감 = `max(공지 종료, running_at) + 1시간` (`effective_end`).
- 관측이 없으면 공지 기준 그대로 — 제때 끝난 세미나의 조용한 마감은 유지된다.
- 마크업이 깨져 관측이 계속 갱신되는 경우를 막는 상한: `SURVEY_RUNNING_EXTEND_CAP`(6시간).

회귀 테스트: `tests/test_seminar_survey_false_done.py`.
